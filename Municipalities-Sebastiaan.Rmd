...
lm_eq <-  `Vaccination_completely_18` ~ 
  ...
  # Religion
    ...
    #Boeddhist +
    anders 
  # check for confounding distance to GP
    #WeinigStedelijk_55
...
require(reshape2)
ggplot(data = melt(fb_gwr_out[,2:13]), aes(x=variable, y=value)) + geom_boxplot() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
data_compl$ft_ZiektenVanAdemhalingsstelsel_66 = fb_gwr_out$ZiektenVanAdemhalingsstelsel_66 / fb_gwr_out$ZiektenVanAdemhalingsstelsel_66_se

#categorize the t-value to statistical significance
data_compl$ft_ZiektenVanAdemhalingsstelsel_66_cat <- cut(data_compl$ft_ZiektenVanAdemhalingsstelsel_66,
                             breaks=c(min(data_compl$ft_ZiektenVanAdemhalingsstelsel_66), -1.96, 1.96, max(data_compl$ft_ZiektenVanAdemhalingsstelsel_66)),
                             labels=c("sig","nonsig", "sig"))

#plot the significance for fixed kernel model
sig_fb <- mapview::mapview (data_compl, zcol = "ft_ZiektenVanAdemhalingsstelsel_66_cat")



#For Adaptive model
#estimate the t-value for green space availability variable for fixed kernel model
data_compl$at_ZiektenVanAdemhalingsstelsel_66 = ab_gwr_out$ZiektenVanAdemhalingsstelsel_66 / ab_gwr_out$ZiektenVanAdemhalingsstelsel_66_se

#categorize the t-value to statistical significance
data_compl$at_ZiektenVanAdemhalingsstelsel_66_cat <- cut(data_compl$at_ZiektenVanAdemhalingsstelsel_66,
                             breaks=c(min(data_compl$at_ZiektenVanAdemhalingsstelsel_66), -1.96, 1.96, max(data_compl$at_ZiektenVanAdemhalingsstelsel_66)),
                             labels=c("sig","nonsig", "sig"))

#plot the significance for adaptive kernel model
sig_ab <- mapview::mapview(data_compl, zcol = "at_ZiektenVanAdemhalingsstelsel_66_cat")
